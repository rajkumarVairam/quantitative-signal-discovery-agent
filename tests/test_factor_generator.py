...
"""Tests for factor_generator pure utility functions."""

import pytest

from factor_mining_workflow.factor_generator import (
    build_factor_example,
    build_factor_prompt,
    build_factor_template,
...
class TestLoadOutputTemplate:
    ...
    def test_has_factor_template(self):
        assert "factor_template" in load_output_template()

    def test_factor_template_required_fields(self):
        ft = load_output_template()["factor_template"]
...
class TestBuildFactorTemplate:
    def test_returns_string(self):
        assert isinstance(build_factor_template(2), str)

    def test_wrapped_in_json_fence(self):
        result = build_factor_template(2)
        assert result.startswith("```json")
        assert result.rstrip().endswith("```")

    def test_contains_required_fields(self):
        result = build_factor_template(1)
        for field in ("name", "formula", "meaning", "category"):
            assert f'"{field}"' in result

    def test_repeats_for_num_factors(self):
        # Each factor block should appear num_factors times.
        n = 3
        result = build_factor_template(n)
...
class TestBuildFactorExample:
    def test_returns_string(self):
        assert isinstance(build_factor_example(), str)

    def test_wrapped_in_json_fence(self):
        result = build_factor_example()
...
class TestBuildFactorPrompt:
    ...
    def test_includes_request(self, operators):
        prompt = build_factor_prompt(
            ...
            template_block=build_factor_template(2),
        )
        assert "momentum factors" in prompt

    def test_includes_num_factors(self, operators):
        prompt = build_factor_prompt(
            request="x",
            num_factors=7,
            operators_list="",
            template_block=build_factor_template(7),
        )
        assert "Generate 7 factors" in prompt
    ...
    def test_includes_feedback_section_when_provided(self, operators):
        prompt = build_factor_prompt(
            ...
            num_factors=1,
            operators_list="",
            template_block=build_factor_template(1),
    ...
    def test_includes_example_when_provided(self, operators):
        prompt = build_factor_prompt(
            ...
            operators_list="",
            template_block=build_factor_template(1),
            example_block=build_factor_example(),
        )
        assert "example of one valid factor" in prompt
